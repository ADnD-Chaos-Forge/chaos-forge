...
exports.getThac0 = getThac0;
exports.getAttackRoll = getAttackRoll;
exports.getSavingThrows = getSavingThrows;
exports.getClassSaveAdjustments = getClassSaveAdjustments;
exports.getPriesthoodSaveBonus = getPriesthoodSaveBonus;
exports.getSavingThrowsForClass = getSavingThrowsForClass;
exports.getAttacksPerRound = getAttacksPerRound;
exports.getAdjustedWeaponThac0 = getAdjustedWeaponThac0;
exports.getEffectiveWeaponSpeed = getEffectiveWeaponSpeed;
exports.formatDamageWithBonus = formatDamageWithBonus;
...
const WARRIOR_THAC0_CLASSES = ["crusader"];
function getThac0(classGroup, level, classId) {
    const effectiveGroup = classId && WARRIOR_THAC0_CLASSES.includes(classId) ? "warrior" : classGroup;
    let thac0;
...
function getAttackRoll(thac0, targetAC) {
...
function getSavingThrows(classGroup, level) {
 ...
 */
function getClassSaveAdjustments(classId) {
    if (classId === "druid") {
        return { breath: -2 };
    }
    return {};
 ...
 */
function getPriesthoodSaveBonus(priesthoodId, level) {
    const powers = (0, priesthoods_1.getActivePowers)(priesthoodId, level);
    const result = {};
 ...
 */
function getSavingThrowsForClass(classGroup, classId, level, priesthoodId) {
    ...
    return {
        paralyzation: base.paralyzation + (classAdj.paralyzation ?? 0) + (priesthoodAdj.paralyzation ?? 0),
        rod: base.rod + (classAdj.rod ?? 0) + (priesthoodAdj.rod ?? 0),
        petrification: base.petrification + (classAdj.petrification ?? 0) + (priesthoodAdj.petrification ?? 0),
...
// All others: 1/1
function getAttacksPerRound(classGroup, level, isSpecialized = false) {
 ...
 */
function getAdjustedWeaponThac0(baseThac0, strHitAdj, dexMissileAdj, weaponType, proficiencyPenalty, weaponHitBonus = 0) {
    return {
        melee: baseThac0 - strHitAdj - proficiencyPenalty - weaponHitBonus,
        ranged: weaponType !== "melee"
 ...
 */
function getEffectiveWeaponSpeed(baseSpeed, hitBonus, damageBonus = hitBonus) {
 ...
 */
function formatDamageWithBonus(baseDamage, strDmgAdj, weaponDmgBonus = 0) {
